...
export function ValidationStatusBar({ validation, editorView }: ValidationStatusBarProps) {
  ...
  // Deduplicate and limit to 5 line numbers
  const uniqueLines = [...new Set(validation.errors.map((e) => e.line))].slice(0, 5);

  if (validation.valid) {
    return (
      <div className="shrink-0 flex items-center gap-1.5 px-2 py-1 text-[10px]">
