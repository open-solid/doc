  ...
  return (
    <div className="prose-doc max-w-4xl mt-10">
      <Markdown
        ...
        components={{
          h1: ({ children }) => (
            <h1 id={slugify(children)} className="text-3xl font-bold mt-0 mb-6 text-slate-900 dark:text-slate-300 scroll-mt-18">
              {children}
            </h1>
          ),
          h2: ({ children }) => (
            <h2 id={slugify(children)} className="text-2xl font-semibold mt-10 mb-4 text-slate-900 dark:text-slate-300 pb-2 scroll-mt-18">
              {children}
            </h2>
          ),
          h3: ({ children }) => (
            <h3 id={slugify(children)} className="text-xl font-semibold mt-8 mb-3 text-slate-900 dark:text-slate-300 scroll-mt-18">
              {children}
            </h3>
          ),
          h4: ({ children }) => (
            <h4 id={slugify(children)} className="text-lg font-medium mt-6 mb-2 text-slate-900 dark:text-slate-300 scroll-mt-18">
              {children}
            </h4>
          ),
          p: ({ children }) => (
          ...
          strong: ({ children }) => (
            <strong className="font-semibold text-slate-900 dark:text-slate-300">{children}</strong>
          ),
        }}
      >
        {content}
...
function CodeBlock({ className, children }: { className?: string; children?: React.ReactNode }) {
  ...
  if (!match) {
    return (
      <code className="bg-slate-100 dark:bg-slate-800 rounded px-1.5 py-0.5 text-sm font-mono text-slate-800 dark:text-slate-300">
        {children}
      </code>
    );
  }
